...
#   test/ft/run.sh "builds"              # only tests matching a Lua pattern
...
# CE_FT_DATA    the throwaway data directory the run happens in
set -euo pipefail
...
# directory's mods folder, and a data directory inside the repo would therefore make the
# repo contain itself.
data="${CE_FT_DATA:-$HOME/.cache/ce-factorio-test}"
...
# ce-tests carries no prototypes; it is the gate that keeps the fixtures from registering
