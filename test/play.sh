#!/usr/bin/env bash
# Play tier: a real game, laid out for trying the equipment by hand.
#
#   test/play.sh                         # a fresh freeplay game, everything set up
#   CE_SPACE_AGE=1 test/play.sh          # with the expansion as well
#
...
# loads: script changes are picked up by restarting, and so are prototype changes.
#
# What is laid out, by test/play/ce-play: every technology researched, an armour with two
# tiers of arm in it, a ring of ghosts to walk into, and a car, a tank and a spidertron
# standing to the east, each with its own ghosts. The tank and the spidertron carry arms of
# their own and the belts to feed them; the car has no equipment grid, which is the case
# where a driver's armour goes quiet and nothing takes over.
#
# CE_FACTORIO     the game binary, if it is not where Steam puts it here
# CE_PLAY_DATA    the data directory the game runs out of (default ~/.cache/ce-play)
set -euo pipefail

root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
factorio="${CE_FACTORIO:-/home/sparr/Games/Steam/steamapps/common/Factorio/bin/x64/factorio}"
data="${CE_PLAY_DATA:-$HOME/.cache/ce-play}"
...
ln -sfn "$root/test/play/ce-play" "$data/mods/ce-play"
